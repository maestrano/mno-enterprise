...
module MnoEnterprise
  describe Jpi::V1::AppInstancesController, type: :controller do
    ...
    describe 'GET #index' do
      ...
      before { stub_api_v2(:get, '/app_instances', [app_instance], [:app], {filter: {owner_id: organization.id, 'status.in': MnoEnterprise::AppInstance::ACTIVE_STATUSES.join(','), 'fulfilled_subscription': true }}) }
      ...
      describe 'all' do
        it {
          ...
          assert_requested(:get, api_v2_url('/app_instances', [:app], {_locale: I18n.locale, filter: {owner_id: organization.id, 'status.in': MnoEnterprise::AppInstance::ACTIVE_STATUSES.join(','), 'fulfilled_subscription': true }}))
        }
      end

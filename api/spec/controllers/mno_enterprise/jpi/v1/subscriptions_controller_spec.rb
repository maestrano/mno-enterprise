...
module MnoEnterprise
  describe Jpi::V1::SubscriptionsController, type: :controller do
    ...
    # Stub params for subscriptions
    let(:subscription_stub_included) { [:'product_pricing.product', :product, :product_contract, :organization, :user, :'license_assignments.user', :'product_instance.product'] }
    ...
    describe 'GET #show' do
      ...
      before do
        stub_api_v2(
          :get, "/subscriptions", subscription, subscription_stub_included,
          {filter: {organization_id: organization.id, id: subscription.id, subscription_status_in: 'visible'}, 'page[number]' => 1, 'page[size]' => 1, '_metadata[organization_id]' => organization.id}
        )
      end
      before { sign_in user }
    ...
    describe 'POST #create' do
      ...
      context 'staged subscription' do
        before do
          stub_api_v2(
            :get, "/subscriptions", subscription, subscription_stub_included,
            {filter: {organization_id: organization.id, id: subscription.id, subscription_status_in: 'staged'}, 'page[number]' => 1, 'page[size]' => 1, '_metadata[organization_id]' => organization.id}
          )
        end
        before { sign_in user }
      ...
      context 'normal subscription' do
        before { stub_api_v2(:get, "/subscriptions", subscription, subscription_stub_included, {filter: {organization_id: organization.id, id: subscription.id, subscription_status_in: 'visible'}, 'page[number]' => 1, 'page[size]' => 1, '_metadata[organization_id]' => organization.id}) }
        before { sign_in user }
